...
##  bt_error($message)
sub bt_error {
    print STDOUT ${bencode_dict({ 'failure reason' => $_[0] })};
...
sub bt_peer_started {
    ...
    $dbh->do(QSTR->{'info_add'}, {}, Socket::inet_ntoa($iaddr),
...
sub bdecode_dict {
    ...
    my $s = \$_[0];
    my(%d,$k,$t,$l,$n);
    substr($$s,0,1,'') eq 'd' || return;
    while (substr($$s,0,1) ne 'e' && $$s ne '') {
	($n = index($$s,':'))      > 0 || return;
	($n = substr($$s,0,$n,'')) > 0 || return;
	substr($$s,0,1,'');
	$k = substr($$s,0,$n,'');
	$t = BENCODING_KEY_TYPES->{$k} ||
	  ## if unknown key, attempt to detect the data type
	  (($n = substr($$s,0,1)) eq 'd'
	    ? TYPE_DICT
	    : $n eq 'l'
	      ? ($n = substr($$s,1,1)) eq 'd'
	...
	if (substr($$s,0,1) eq 'e' || $$s eq '') {
	    return; ## missing value; premature string end
	}
	elsif ($t == TYPE_STRING) {
	    ($n = index($$s,':'))      > 0 || return;
	    ($n = substr($$s,0,$n,'')) > 0 || return;
	    ...
	    substr($$s,0,1,'') eq 'i' || return;
	    ($n = index($$s,'e')) > 0 || return;
	    $d{$k} = substr($$s,0,$n,'');
	    substr($$s,0,1,'');
	}
	elsif ($t == TYPE_DICT) {
	    $d{$k} = bdecode_dict($$s) || return;
	}
	else { ## TYPE_{DICT,STRING,NUMBER}_LIST
	    $d{$k} = $l = [];
	    substr($$s,0,1,'') eq 'l' || return;
	    if ($t == TYPE_DICT_LIST) {
		push(@$l, bdecode_dict($$s) || return)
		  while (substr($$s,0,1) ne 'e' && $$s ne '');
	    }
	    elsif ($t == TYPE_STRING_LIST) {
		while (substr($$s,0,1) ne 'e' && $$s ne '') {
...
## Perform a non-blocking connect to client and speak simple BitTorrent protocol
## (alarm() is not safe for use in Apache 1.3/mod_perl; use select() instead)
## Under high system loads, Apache will begin to be backlogged while bad
## connections consume resources before timing out.  Lower the timeout or
## disable CHECK_PEER.
...
sub scan_torrent_dir {
    ...
    require('File/Find.pm') && require('Digest/SHA1.pm') unless MOD_PERL;
...
sub process_torrent_files {
    ...
    $$metainfo{'announce'} eq TRACKER_URL
      || ((print STDERR "$File::Find::name does not announce this tracker (".
			TRACKER_URL.")\n"),
	   return);
...
sub read_torrent_file {
    ...
    local $/;
    $torrent_path =~ m%[^/]\.torrent$%
      && $torrent_path !~ m%^\.{1,2}/|/\.{1,2}/|/\.{1,2}$|\0%
      && open($FH,'<'.$torrent_path)
      && binmode($FH,':raw')
      && ($size = (stat($FH))[7])
      && (-f _)
      && $size < 524288  ## (512 kB)
      && ($/ = \$size)
      && ($torrent = bdecode_dict(<$FH>))
      && $$torrent{'announce'} ne ''
      && ($info = $$torrent{'info'})
      && defined($$info{'piece length'})
      && defined($$info{'pieces'})
      && (defined($$info{'length'}) ^ defined($$info{'files'}))
      && close($FH)
      || return;
    defined($$info{'name'})
      ? length($$info{'name'}) <= 92 || return
      : ($$info{'name'} = '');
    return $torrent;
}
...
sub parse_query_string {
    ...
    my $input = defined($_[0])?$_[0]:'';# copy query string because we modify it
